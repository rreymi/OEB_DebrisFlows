function [crest_loc, bottom_loc] = get_wave_crest(ptcloud, img_pixels, bbox)
    ...
    kb = 5; % movmean params, should be fine
    kf = 5; % movmean params, should be fine
    ...
    % plot lines and derivatives
    if plot_lines
        figure
        for ii=1:num_lines
            subplot(3,1,1)
            %plot(lines_y(:,ii), lines_z(:,ii),'r')
            plot(lines_y(:,ii), lines_z_smoothed(:,ii))
            hold on
            %plot(lines_y(bottom_idx_inall,ii),lines_z_smoothed(bottom_idx_inall,ii),'o')
            %plot(lines_y(crest_idx_inall,ii),lines_z_smoothed(crest_idx_inall,ii),'o')
            
            subplot(3,1,2)
            hold on
            plot(lines_y(:,ii), dz_dy(:,ii))
            
            subplot(3,1,3)
            plot(lines_y(:,ii), d2z_dy2(:,ii))
            hold on
            yline(0,'k')
            %plot(lines_y(bottom_idx_inall,ii), d2z_dy2(bottom_idx_inall,ii),'o')
            %plot(lines_y(crest_idx_inall,ii), d2z_dy2(crest_idx_inall,ii),'o')
        end
    end
    ...
    % the min of the first derivative is the steepest part of the wave
    [min_dz_dy, idx_min_dz_dy] = min(dz_dy);
    
    % find bottom
    % the bottom is where the second derivative goes back to ~0
    bottom_loc =zeros(num_lines,3);
    all_indices = 1:length(d2z_dy2);
    for i=1:num_lines
        bottom_candidates = d2z_dy2(idx_min_dz_dy(i):end,i);
        bottom_candidates_idx = all_indices(idx_min_dz_dy(i):end);
        
        [~,idx_max] = max(bottom_candidates);
        if not(idx_max == length(bottom_candidates))
            curr_gradient = -99;
            curr_value = 99;
            curr_idx = idx_max;
            while (curr_gradient < 0) && (curr_value > 0) && (curr_idx < length(bottom_candidates_idx)-1) % stops as soon as one of these conditions is not true anymore
                curr_idx = curr_idx + 1;
                curr_gradient = bottom_candidates(curr_idx+1) - bottom_candidates(curr_idx);
                curr_value = bottom_candidates(curr_idx);
            end
            
            if curr_idx >= length(bottom_candidates_idx)-1
                bottom_loc(i,1) = NaN;
                bottom_loc(i,2) = NaN;
                bottom_loc(i,3) = NaN;
            else
    ...
    for i=1:num_lines
        crest_candidates = d2z_dy2(1:idx_min_dz_dy(i),i);
        crest_candidates_idx = all_indices(1:idx_min_dz_dy(i));
        ...
        if not(idx_min == 1)
            ...
            while (curr_gradient > 0) && (curr_value < 0) && (curr_idx > 2) % stops as soon as one of these conditions is not true anymore
                curr_idx = curr_idx - 1;
                curr_gradient = crest_candidates(curr_idx-1) - crest_candidates(curr_idx);
            ...
            if curr_idx <= 2
                ...
                %crest_idx = max(find(crest_candidates < 0)+1);
