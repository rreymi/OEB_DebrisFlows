function project_ptcloud_on_img_to_video(allStats, pathIMG, pathPtCloud, cameraParams, tform, spacing, zCutoff, xLim, yLim, fps, save_path, video_name)
...
%   video_name    - output video file name (default 'ptcloud_on_img.mp4')
    ...
    for k = 1:numel(frames_img)
        ...
        % Title
        title(ax, sprintf('TrackID %d - Frame %d (%d/%d)',trackid, frameImg, k, numel(frames_img)));
        drawnow;
